    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          ...
          case `/sub/${userID}`:
            if (client === "sfa") {
              const BestPingSFA = await getSingboxConfig(
                env,
                host,
                client,
                false,
          ...
          case `/warpsub/${userID}`:
            ...
            if (client === "singbox" || client === "hiddify") {
              const singboxWarpConfigs = await getSingboxConfig(
                env,
                host,
                client,
                true,
...
async function updateDataset(env, Settings, resetSettings) {
  ...
  if (!resetSettings) {
    try {
      currentProxySettings = await env.bpb.get("proxySettings", {
        type: "json",
      });
    } catch (error) {
      console.log(error);
  ...
  const proxySettings = {
    remoteDNS:
      (Settings
        ...
        : currentProxySettings?.remoteDNS) || "https://dns.google/dns-query",
    localDNS:
      (Settings ? Settings.get("localDNS") : currentProxySettings?.localDNS) ||
      "8.8.8.8",
    lengthMin:
      (Settings
        ? Settings.get("fragmentLengthMin")
        : currentProxySettings?.lengthMin) || "10",
    lengthMax:
      (Settings
        ? Settings.get("fragmentLengthMax")
        : currentProxySettings?.lengthMax) || "30",
    intervalMin:
      (Settings
        ? Settings.get("fragmentIntervalMin")
        : currentProxySettings?.intervalMin) || "1",
    intervalMax:
      (Settings
        ? Settings.get("fragmentIntervalMax")
        : currentProxySettings?.intervalMax) || "2",
    fragmentPackets:
      (Settings
        ? Settings.get("fragmentPackets")
        : currentProxySettings?.fragmentPackets) || "tlshello",
    blockAds:
    ...
    bypassChina:
      (Settings
        ? Settings.get("bypass-china")
        : currentProxySettings?.bypassChina) || false,
    blockUDP443:
      (Settings
        ? Settings.get("block-udp-443")
        : currentProxySettings?.blockUDP443) || false,
    cleanIPs:
      (Settings
    ...
    vlessConfigs:
      (Settings
        ? Settings.get("vlessConfigs")
        : currentProxySettings?.vlessConfigs) || true,
    trojanConfigs:
      (Settings
        ? Settings.get("trojanConfigs")
        : currentProxySettings?.trojanConfigs) || false,
    outProxy: (Settings ? chainProxy : currentProxySettings?.outProxy) || "",
    outProxyParams:
      (chainProxy
        ? extractChainProxyParams(chainProxy)
        : currentProxySettings?.outProxyParams) || "",
    wowEndpoint:
      (Settings
        ? Settings.get("wowEndpoint")?.replaceAll(" ", "")
        : currentProxySettings?.wowEndpoint) ||
      "188.114.98.183:955,188.114.98.99:4233,188.114.99.223:2506,162.159.192.175:891,188.114.98.1:1010,162.159.192.0:955,162.159.192.187:7559",
    warpEndpoints:
      (Settings
    ...
    hiddifyNoiseMode:
      (Settings
        ? Settings.get("hiddifyNoiseMode")
        : currentProxySettings?.hiddifyNoiseMode) || "m4",
    nikaNGNoiseMode:
      (Settings
        ? Settings.get("nikaNGNoiseMode")
        : currentProxySettings?.nikaNGNoiseMode) || "quic",
    noiseCountMin:
      (Settings
        ? Settings.get("noiseCountMin")
        : currentProxySettings?.noiseCountMin) || "10",
    noiseCountMax:
      (Settings
        ? Settings.get("noiseCountMax")
        : currentProxySettings?.noiseCountMax) || "15",
    noiseSizeMin:
      (Settings
        ? Settings.get("noiseSizeMin")
        : currentProxySettings?.noiseSizeMin) || "5",
    noiseSizeMax:
      (Settings
        ? Settings.get("noiseSizeMax")
        : currentProxySettings?.noiseSizeMax) || "10",
    noiseDelayMin:
      (Settings
        ? Settings.get("noiseDelayMin")
        : currentProxySettings?.noiseDelayMin) || "1",
    noiseDelayMax:
      (Settings
        ? Settings.get("noiseDelayMax")
        : currentProxySettings?.noiseDelayMax) || "1",
    warpPlusLicense:
      (Settings
        ? Settings.get("warpPlusLicense")
        : currentProxySettings?.warpPlusLicense) || "",
    customCdnAddrs:
      (Settings
        ? Settings.get("customCdnAddrs")?.replaceAll(" ", "")
        : currentProxySettings?.customCdnAddrs) || "",
    customCdnHost:
      (Settings
        ? Settings.get("customCdnHost")?.trim()
        : currentProxySettings?.customCdnHost) || "",
    customCdnSni:
      (Settings
        ? Settings.get("customCdnSni")?.trim()
        : currentProxySettings?.customCdnSni) || "",
    bestVLESSTrojanInterval:
      (Settings
        ? Settings.get("bestVLESSTrojanInterval")
        : currentProxySettings?.bestVLESSTrojanInterval) || "30",
    bestWarpInterval:
      (Settings
        ? Settings.get("bestWarpInterval")
        : currentProxySettings?.bestWarpInterval) || "30",
...
async function buildWarpOutbounds(env, client, proxySettings, warpConfigs) {
  ...
  warpEndpoints.split(",").forEach((endpoint, index) => {
    if (client === "xray" || client === "nikang") {
      ...
      client === "nikang" &&
        Object.assign(xrayOutbound.settings, {
...
async function buildXrayDNSObject(
  remoteDNS,
  localDNS,
  blockAds,
  bypassIran,
  ...
  if (dohHost && isDOHDomain) {
    ...
    if (!isWorkerLess)
      dnsObject.hosts[dohHost] = [...resolvedDOH.ipv4, ...resolvedDOH.ipv6];
  }
  ...
  if (
    !isWorkerLess &&
    localDNS !== "localhost" &&
    (bypassIran || bypassChina || bypassLAN)
  ) {
    let localDNSServer = {
      address: localDNS,
      domains: [],
    ...
    bypassLAN &&
      localDNSServer.domains.push("geosite:private") &&
      localDNSServer.expectIPs.push("geoip:private");
    bypassIran &&
...
async function buildWorkerLessConfig(
  remoteDNS,
  localDNS,
  lengthMin,
  lengthMax,
  ...
  fragConfig.dns = await buildXrayDNSObject(
  ...
  fragConfig.routing.rules = buildXrayRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
...
async function getFragmentConfigs(env, hostName) {
  ...
  config.dns = await buildXrayDNSObject(
    remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
  ...
  balancerConfig.routing.rules = buildXrayRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  const domainAddressesRules = Addresses.filter((address) =>
    isDomain(address),
    ...
    for (let portIndex in ports.filter((port) =>
      ...
      for (let index in Addresses) {
        ...
        if (vlessConfigs && i === 0) {
          ...
          outbound = buildXrayVLESSOutbound(
            "proxy",
            addr,
            port,
            userID,
        ...
        if (trojanConfigs && !outbound) {
          ...
          outbound = buildXrayTrojanOutbound(
            "proxy",
            addr,
            port,
            trojanPassword,
  ...
  const workerLessConfig = await buildWorkerLessConfig(
    remoteDNS,
    localDNS,
    lengthMin,
    lengthMax,
...
async function getXrayWarpConfigs(env, client) {
  ...
  const xrayWarpOutbounds = await buildWarpOutbounds(
    env,
    client,
    proxySettings,
    warpConfigs,
  );
  const xrayWoWOutbounds = await buildWoWOutbounds(
    env,
    client,
    proxySettings,
    warpConfigs,
  ...
  xrayWarpConfig.dns = await buildXrayDNSObject(
    "1.1.1.1",
    localDNS,
    blockAds,
    bypassIran,
  ...
  xrayWarpConfig.routing.rules = buildXrayRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  xrayWarpBestPing.remarks =
    client === "nikang" ? "🔴 Warp Pro Best Ping" : "🔴 Warp Best Ping";
  xrayWarpBestPing.dns = await buildXrayDNSObject(
    "1.1.1.1",
    localDNS,
    blockAds,
    bypassIran,
  ...
  xrayWoWConfigTemp.dns = await buildXrayDNSObject(
    "1.1.1.1",
    localDNS,
    blockAds,
    bypassIran,
  ...
  xrayWoWConfigTemp.routing.rules = buildXrayRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  xrayWoWBestPing.remarks =
    client === "nikang" ? "🔴 WoW Pro Best Ping" : "🔴 WoW Best Ping";
...
async function buildClashDNS(
  ...
  if (DNSNameserver && isDOHDomain) {
    ...
    dns["hosts"] = {
      [`${DNSNameserver}`]: [...resolvedDOH.ipv4, ...resolvedDOH.ipv6],
    };
  }
...
function buildClashRoutingRules(
  localDNS,
  blockAds,
  bypassIran,
  bypassChina,
...
function buildClashWarpOutbound(
  remark,
  ipv6,
  privateKey,
  publicKey,
...
async function getClashConfig(env, hostName, isWarp) {
  ...
  config.dns = await buildClashDNS(
    isWarp ? "1.1.1.1" : remoteDNS,
    localDNS,
    blockAds,
    bypassIran,
    blockPorn,
  ...
  config.rules = buildClashRoutingRules(
    localDNS,
    blockAds,
    bypassIran,
    bypassChina,
  ...
  if (isWarp) {
    ...
    const clashWarpOutbounds = await buildWarpOutbounds(
      env,
      "clash",
      proxySettings,
      warpConfigs,
    );
    const clashWOWpOutbounds = await buildWoWOutbounds(
      env,
      "clash",
      proxySettings,
      warpConfigs,
    ...
    clashWOWpOutbounds.forEach((outbound) => {
      outbound["name"].includes("WoW") &&
        wowOutboundRemarks.push(outbound["name"]);
    });
  } else {
  ...
  config["proxy-groups"][0].proxies = isWarp
    ? [
        "🔴 Warp Best Ping",
        "🔴 WoW Best Ping",
        ...warpOutboundsRemarks,
        ...wowOutboundRemarks,
      ]
    : ["🔴 Best Ping", ...outboundsRemarks];

  config["proxy-groups"][1].proxies = isWarp
    ? warpOutboundsRemarks
    : outboundsRemarks;
  (config["proxy-groups"][1].name = isWarp
    ? `🔴 Warp Best Ping`
    : `🔴 Best Ping`),
    isWarp &&
...
function buildSingboxDNSRules(
  ...
  let rules = [
    {
      domain: ["www.gstatic.com", ...outboundDomains],
      server: "dns-direct",
    },
    {
      outbound: "any",
...
function buildSingboxRoutingRules(
  blockAds,
  bypassIran,
  bypassChina,
  blockPorn,
...
async function getSingboxConfig(env, hostName, client, isWarp, isFragment) {
  ...
  if (outProxy && !isWarp) {
    ...
    try {
      ...
      isDomain(chainProxyOutbound.server) &&
        outboundDomains.push(chainProxyOutbound.server);
  ...
  if (isWarp) {
    const warpOutbounds = await buildWarpOutbounds(
      env,
      client,
      proxySettings,
      warpConfigs,
    );
    const WOWOutbounds = await buildWoWOutbounds(
      env,
      client,
      proxySettings,
      warpConfigs,
    ...
    config.outbounds[1].tag =
      client === "hiddify" ? "🔴 Warp Pro Best Ping" : "🔴 Warp Best Ping";
    config.outbounds[2].tag =
      client === "hiddify" ? "🔴 WoW Pro Best Ping" : "🔴 WoW Best Ping";
  ...
  for (let i = 0; i < protocolsNo && !isWarp; i++) {
    ports
      .filter(
        (port) =>
          (defaultHttpsPorts.includes(port) && isFragment) || !isFragment,
      )
      .forEach((port) => {
        totalAddresses.forEach((addr, index) => {
          let VLESSOutbound, TrojanOutbound;
          ...
          const configIndex = isCustomAddr
  ...
  config.dns.rules = buildSingboxDNSRules(
    blockAds,
    bypassIran,
    bypassChina,
    blockPorn,
    new Set(outboundDomains),
  );
  const { rules, rule_set } = buildSingboxRoutingRules(
    blockAds,
    bypassIran,
    bypassChina,
    blockPorn,
  ...
  ports.forEach((port) => {
    totalAddresses.forEach((addr, index) => {
      ...
      const trojanRemark = encodeURIComponent(
        generateRemark(
          proxyIndex + totalAddresses.length * ports.length,
          port,
          addr,
          cleanIPs,
          "TROJAN",
